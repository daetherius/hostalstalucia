...
class AbcsController extends MyController{
	...
	function admin_agregar() {
			...
			if($return = $this->m[0]->saveAll($this->data,array('validate'=>true))){
				foreach($habtmModels as $habtmModel){
					if(isset($habtmData[$habtmModel]))
						$this->m[0]->$habtmModel->savehabtm($habtmData[$habtmModel],$this->m[0]->id);
				}
		...
		foreach($habtmModels as $habtmModel){
			$habtmData = $this->m[0]->$habtmModel->find(null,'list');
	...
	function admin_editar($id) {
		...
		if(empty($this->data)){
			...
			foreach($habtmModels as $habtmModel){
				if(isset($this->data[$habtmModel]) && $this->data[$habtmModel] && method_exists($this->$habtmModel,'savehabtm'))
					$habtmData[$habtmModel] = $this->data[$habtmModel];unset($this->data[$habtmModel]);
			}
...
/*
	function admin_activar($id,$exclusive = false) {
		...
		if($data = $this->m[0]->read(array('id','activo'),$id)){
			...
			$this->m[0]->create();
			$this->m[0]->id = $id;
			if($this->m[0]->saveField('activo',!$data[$this->uses[0]]['activo']))
				$msg = 'ok';
		}
