...
var Pulsembox = new Class({
		...
		if(this.isImageURL(url)){
			...
			pboximg.addEvents({
				'load': function(){
					...
					new Element('img#PboxImage',{
						src:url,
						alt:caption
					}).inject('PboxImageClose');
					
					this.imgFx = new Fx.Tween('PboxImage',{ property:'opacity', duration:this.duration, link:'cancel' }).set(0);
				...
				//// Calcular Dimensiones de Imagen
					var padding = $('PboxImage').measure(function (){ return this.getPosition('PboxPad'); });
					var yplus = 46 + 10 + 68; // 46px (PboxHeader) + 10px (Margen) + 68 Caption
					var x = window.getWidth() - 20; // 20 margen lateral
					var y = window.getHeight() - yplus;
	
					var imgW = pboximg.width;
					var imgH = pboximg.height;
	...
	/*
					$('PboxImage').setStyle('height',imgH);
					$('PboxImage').setStyle('width',imgW);
	*/
					
					this.boxWidth = imgW + (padding.x*2); // PboxImage + (2*2px (Border)) + (padding.x*2)
					this.boxHeight = imgH + (padding.y*2) + yplus - 10;// yplus - 10 (margen, afuera del box, por tanto, no se considera aqui)
					
					if(this.boxWidth - (padding.y*2) < 264 ){
						$('PboxImage').setStyle('width',this.boxWidth);
						this.boxWidth =  264 + (padding.y*2);
					}
	...
	getInfo: function(idx, image, id, controlHTML){
		return {
			idx: idx,
			caption: image.title || image.name || "",
			url: image.href,
