		...
		echo $html->div('sidebar');
			...
			foreach($modules as $cntrllr => $mod){
				if(isset($mod['admin']) && $mod['admin']){
					echo $html->link($html->tag('span',ucfirst($mod['admin']['label']), array('class'=>$mod['admin']['class'])),array('controller'=>$cntrllr, 'action'=>'index','admin'=>true),array('class'=>in_array($cntrllr,$breaks) ? 'withbreak':''));
				}
			}
